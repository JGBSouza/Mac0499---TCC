...
\posterbox[adjusted title = Contribuição no kernel Linux, smallmargins, equal height group = toprow, colback=white, before upper=\justifying]
          {name=contexto, below=titlebox, column=1, span=6}{

    \noindent Desenvolvimento de maneira contínua e altamente colaborativa. O fluxo de submissão de contribuições, acompanhamento das respostas, versões e estados dos patches é feita exclusivamente via lista de e-mails públicas, dependendo de verificações manuais ou configurações de ferramentas externas  que acrescentam considerável overhead para colaboradores.\\
}
...
\posterbox[adjusted title = Kworkflow, smallmargins, equal height group = toprow, colback=white, before upper=\justifying]
    ...
    \noindent Ferramenta de terminal para automação de tarefas comuns no desenvolvimento do kernel Linux. Funciona como um hub, integrando soluções locais e externas. Para melhoria da cobertura do sistema, adicionamos dois novos sistemas, \textbf{kw manage-contacts}, para gerenciamento de contatos e grupos de email e o \textbf{kw patch-track}, para rastreio, atualização de estado e visualizações de submissões e respostas dos patches submetidos\\
}

%%%%%%%% kw manage-contacts funcionalidades %%%%%%%%

\posterbox[adjusted title = Funcionalidades do kw manage-contacts, smallmargins, equal height group = toprow, colback=white, before upper=\justifying]
          {name=manage-contacts, below=contexto, column=1, span=6} {
  \begin{itemize}
...
\posterbox[adjusted title = Funcionalidades do kw patch-track, smallmargins, equal height group = toprow, colback=white, before upper=\justifying]
          {name=patch-track, below=kworkflow, column=7, span=6} {
    \begin{itemize}
...
\posterbox[adjusted title = Resultados, smallmargins, equal height group = toprow, colback=white, before upper=\justifying]
          {name=resultados, below=manage-contacts, column=1, span=6} {
  \begin{itemize}
    \item Acelera fluxo de submissão com uso de grupos de destinatários além de aumentar a corretude e permitir visualização de submissões
    \item Melhora visibilidade do estado e evolução dos patches diretamente no fluxo do kw.
    \item Reduz tempo gasto consultando históricos e interações na mailing list.
    \item Reduz perda de contexto ao manter registro local das revisões.
    \item Minimiza alternância entre ferramentas e consultas externas.
  \end{itemize}
}

%%%%%%%% Próximos passos %%%%%%%%

\posterbox[adjusted title = Próximos passos, smallmargins, equal height group = toprow, colback=white, before upper=\justifying]
          {name=proximos-passos, below=patch-track, column=7, span=6} {
